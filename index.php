			...
			<div class="col-md-12">
				<div class="card">
					<div class="card-header d-flex justify-content-between align-items-center">
						...
						<div>
							<!-- Ícone de sol  -->
							<i class="fas fa-sun fa-2x fs-5 float-right text-dark btn-theme light mx-4" role="button"
							   data-bs-toggle="tooltip" data-bs-placement="left" title="Alternar tema"></i>
							<!-- Ícone para mudar o layout  da lista  -->
							<i class="fas fa-list fa-2x fs-5 float-right text-dark btn-layout light grid" role="button"
							   data-bs-toggle="tooltip" data-bs-placement="left" title="Mudar para grade"></i>
						</div>
					</div>
					<div class="card-body">
						...
						<div class="row">
							<div class="col-md-12">
								...
								<!-- Remover -->
								<a href="#" @click="removeFavorite(favorite)" class="text-decoration-none text-dark"><i
						...
						<div>
							...
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								<li v-for="history in historic" class="nav-item d-flex align-items-center">
									<a :href="history.name" class="nav-link text-secondary" :id="history.name"
									   data-toggle="tab" role="tab" aria-controls="home" aria-selected="true"
									   target="_blank"><i :class="history.type"></i> {{ history.name }}</a>
								</li>
							</ul>
							<!-- Layout de lista -->
								...
								<li v-for="(file, index) in files" class="list-group-item element-item border-0">
									...
									<div :class="['row','file-item', index % 2 === 0 ? 'bg-light' : 'bg-white']">
										<div class="col-md-12 d-flex justify-content-between p-0">
                                                <span>
                                                    <i :class="[file.type, (file.type.includes('fa-folder') ? 'text-warning': 'text-secondary')]"></i> <a
															@click="setHistory({name:file.name, type:file.type})"
															:href="file.name" :title="file.name"
															class="text-decoration-none text-dark"
															target="_blank">{{ file.name }}</a>
                                                </span>
											<button v-show="!favorites.includes(file.name)" @click="addFavorite(file)"
													class="btn btn-link p-0" data-bs-toggle="tooltip"
													data-bs-placement="bottom" title="Adicionar aos favoritos">
								...
								<div v-for="file in files" class="col-md-4">
									<div class="card item-grid m-1">
										<div class="card-body row">
											...
											<div class="col-2">
												<button v-show="!favorites.includes(file.name)"
														@click="addFavorite(file)" class="btn btn-link p-0"
					...
					<div class="card">
						<div class="card-footer">
							...
							<small> <i class="fa fa-copyright"></i> <span class="px-2">Copyright 2022</span>
								<span>-</span> <span class="px-2">Desenvolvido por</span> <span>Renzo Nogueira</span>
							</small>
							</p>
						</div>
					</div>
